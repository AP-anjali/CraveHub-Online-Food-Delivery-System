        ...
        { showSearch && userData.role == "user" &&
            ...
            <div className='w-[90%] h-[70px] bg-white shadow-xl rounded-lg items-center gap-[20px] flex fixed top-[80px] left-[5%] md:hidden'>
                ...
                <div className='w-[80%] flex items-center gap-[10px]'>
                    ...
                    <input onChange={(e) => setQuery(e.target.value)} value={query} type="text" placeholder='search delicious food...' className='px-[10px] text-gray-700 outline-0 w-full' />
                </div>
            </div>
        }

        <h1 className='text-3xl font-bold mb-2 text-[#ff4d2d]'>CraveHub</h1>
        ...
        <div className='flex items-center gap-4'>
                ...
                <>
                    {
                        myShopData && 
                        <>
                            <button onClick={() => navigate("/add-food-item")} className='hidden md:flex items-center gap-1 p-2 cursor-pointer rounded-full bg-[#ff4d2d]/10 text-[#ff4d2d]'>
                                <FaPlus size={20} />
                                <span>Add Food Item</span>
                            </button>

                            <button onClick={() => navigate("/add-food-item")} className='md:hidden flex items-center p-2 cursor-pointer rounded-full bg-[#ff4d2d]/10 text-[#ff4d2d]'>
                                <FaPlus size={20} />
                            </button>
                        </>
                    }
...

            {
                ...
                <div className={`fixed top-[80px] right-[10px] ${userData.role == "deliveryBoy" ? "md:right-[20%] lg:right-[30%]" : "md:right-[10%] lg:right-[25%]"} w-[180px] bg-white shadow-2xl rounded-xl p-[20px] flex flex-col gap-[10px] z-[9999]`}>
                    ...
                    {
                        userData.role == "user" &&
                        <div onClick={() => navigate("/my-orders")} className='md:hidden text-[#ff4d2d] font-semibold cursor-pointer'>My Orders</div>
                    }
