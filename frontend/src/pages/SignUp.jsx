...
function SignUp() {
  ...
  const handleGoogleAuth = async () => {
    if(!mobile)
    {
        return setErr("mobile number is required.. fill only that field..!");
    }
  ...
  return (
    <div className='min-h-screen w-full flex items-center justify-center p-4' style={{backgroundColor:bgColor}}>
        <div className={`bg-white rounded-xl shadow-lg w-full max-w-md p-8`} style={{border: `1px solid ${borderColor}`}}>
            ...
            <div className='mb-4'>
                ...
                <div className='flex gap-2'>
                    {roles.map((currentRole) => (
                        <button className='flex-1 border rounded-lg px-3 py-2 text-center font-medium transition-colors cursor-pointer' onClick={() => setRole(currentRole)} style={role== currentRole ? {backgroundColor:primaryColor, color : "white"} : {border:`1px solid ${primaryColor}`, color: primaryColor}}>
                            {currentRole}
                        </button>
                    ))}
                </div>
            ...
            {/* google button */}
            <button onClick={handleGoogleAuth} className='w-full mt-4 flex items-center justify-center gap-2 border rounded-lg px-4 py-2 cursor-pointer transition duration-200 border-gray-400 hover:bg-gray-100'>
                <FcGoogle size={20} /> <span>Sign up with Google</span>
            </button>

            <p className='text-center mt-6'>
                Already have an account..? <span className='text-[#ff4d2d] cursor-pointer' onClick={() => navigate("/signin")}>Sign In</span>
            </p>
