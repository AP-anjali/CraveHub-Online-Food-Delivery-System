...
function CheckOut() {
  ...
  const getAddressByLatLng = async (lat, lng) => {
    try
    {
        const result = await axios.get(`https://api.geoapify.com/v1/geocode/reverse?lat=${lat}&lon=${lng}&format=json&apiKey=${GEOAPIFY_KEY}`);
  ...
  const getLatLngByAddress = async () => {
    ...
    try
    {
        const result = await axios.get(`https://api.geoapify.com/v1/geocode/search?text=${encodeURIComponent(addressInput)}&format=json&apiKey=${GEOAPIFY_KEY}`);
  ...
  return (
    <div className='min-h-screen bg-[#fff9f6] flex items-center justify-center p-6'>
        ...
        <div className='w-full max-w-[900px] bg-white rounded-2xl shadow-xl p-6 space-y-6'>
            ...
            {/* search location and current location and map section */}
            <section>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                    ...
                    <div onClick={() => setPaymentMethod("online")} className={`flex items-center gap-3 rounded-xl border p-4 text-left transition ${paymentMethod === "online" ? "border-[#ff4d2d] bg-orange-50 shadow" : "border-gray-200 hover:border-gray-300"}`}>
                        ...
                        <div>
                            ...
                            <p className='text-xs text-gray-500'>Pay Online Securely</p>
                        </div>
